...
void write_to_log(const string& message) {
    ...
    ofstream log_file("log_report_ROAR_largeDataset_modified_trivialCombination.txt", ios_base::app);
    if (log_file.is_open()) { // Check if file stream successfully opened
        ...
        cerr << "[ERROR] Unable to open log_report_ROAR_largeDataset_modified_trivialCombination.txt for writing!" << endl;
    }
}
...
int main() {
    ...
    for (int i = 0; i < number_execution; i++) {
        ...
        ofstream csv_file("trp_results_ROAR_largeDataset_modified_trivialCombination.csv", ios_base::app);
