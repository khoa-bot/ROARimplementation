...
void write_to_log(const string& message) {
    ...
    ofstream log_file("log_report_ROAR_largeDataset_modified.txt", ios_base::app);
    if (log_file.is_open()) { // Check if file stream successfully opened
        ...
        cerr << "[ERROR] Unable to open log_report_ROAR_largeDataset_modified.txt for writing!" << endl;
    }
}
    ...
    while (!stop_flag.load(std::memory_order_relaxed)) {
        ...
        for (int node : removed_nodes) {
            ...
            if (i_suf < p_size - 2) {
                ...
                for (int j = i_suf + 2; j < p_size ; ++j) {//modified to include the last node for reversal
                    double dist = dm(partial_tour[j-1], partial_tour[j]);
...
int main() {
    ...
    vector<TSPInstance> datasets = {
        ...
        {"fma21553", 21553, 1000, true, false},
        {"usa13509", 13509, 1000, true, false},
        // {"fl1400", 1400, 1000, true, false},
        ...
        {"lsb22777", 22777, 1000, true, false},
        {"pla33810", 33810, 1000, true, false},
        {"boa28924", 28924, 1000, true, false},
        {"bbz25234", 25234, 1000, true, false}
    ...
    for (int i = 0; i < number_execution; i++) {
        ...
        ofstream csv_file("trp_results_ROAR_largeDataset_modified.csv", ios_base::app);
